...
struct node* bin_HEAP_EXTRACT_MIN(struct node* H) {
    if (H == NULL) return NULL;

    struct node *min_node = H, *prev_min = NULL, *prev = NULL, *curr = H;
    // Find the minimum node
    while (curr->sibling != NULL) {
        if (curr->sibling->n < min_node->n) {
    ...
    // Reverse the order of the children of the removed node
    struct node* child = min_node->child;
    ...
    // Merge the children with the rest of the heap
    H = bin_HEAP_UNION(H, child);
